	...
	function findPages(sEntryPage, progressCallback) {
		...
		function checkTestPage(oTestPageConfig) {
			...
			return new Promise(function(resolve, reject) {
				...
				getFile(oTestPageConfig.fullpage).then(function(err, sData) {
					if (err) {
						var text = err;
		...
		function findTestPages(oIFrame) {
			return Promise.resolve(oIFrame.contentWindow.suite()).
				then( function(oSuite) { (oSuite && oSuite.getTestPages() || []) }).
				then( function(aPages) { (decorateWithTestsuite(aPages, oIFrame.src)) }).
				then( function(aPages) { sequence(aPages) }).
				catch( () => [] );
		}
