				...
				windowUtil(qunitHtmlFile, function(testWindow) {
					...
					QUnit.log(function(details) {
						...
						if (config.logAssertions) {
							const testSuiteWithHTMLFilePath = [qunitHtmlFile, details.module, details.name, details.message];
							const testSuite = testSuiteWithHTMLFilePath.slice(1);
							const result = {
								description: details.message,
								suite: details.module && config.logHTMLFilePath ? testSuiteWithHTMLFilePath : testSuite || [],
					...
					QUnit.testDone(function(test) {
						const testSuiteWithHTMLFilePath = [qunitHtmlFile, test];
						const testSuite = testSuiteWithHTMLFilePath.slice(1);
						const result = {
							description: test.name,
							suite: test.module && config.logHTMLFilePath ? testSuiteWithHTMLFilePath : testSuite || [],
