...
class Framework {
	...
	autoDetectType() {
		...
		if (fileContent) {
			...
			if (!configs || configs.length === 0) {
				karma.log("error", ["Could not parse ui5.yaml"]);
			}

			if (!configs[0].type) {
				karma.log("error", ["Project doesn't have a type configured!"]);
			}
	...
	rewriteUrl(url) {
		...
		if (!type) {
			...
			karma.log("error", [`Failed to rewrite url. Type "${type}" is not supported!`]);
		}
