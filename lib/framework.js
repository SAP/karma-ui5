...
class Framework {
	...
	async init({config, logger}) {
		...
		if (config.coverageReporter) {
			this.config.coverageReporter.instrumenterOptions = config.coverageReporter.instrumenterOptions || {};
			this.config.coverageReporter.instrumenterOptions.istanbul = config.coverageReporter.instrumenterOptions.istanbul || {};

			if (!this.config.coverageReporter.instrumenterOptions.istanbul.coverageGlobalScope) {
				this.config.coverageReporter.instrumenterOptions.istanbul.coverageGlobalScope = "(function() { var g=window;while(!g.__karma__&&g!==g.parent){g=g.parent;}; return g; })();";
				this.logger.log("error", "coverageGlobalScope: set");
			}
		}
