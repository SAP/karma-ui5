	...
	rewriteUrlBefore(url) {
		...
		if (type === "library") {
			const resourcesPattern = new RegExp(
				`/base/[${this.replaceLast(srcFolder, "resources")}|${this.replaceLast(testFolder, "resources")}]/`
			);
			const testResourcesPattern = new RegExp(
				`/base/[${this.replaceLast(srcFolder, "test-resources")}|${this.replaceLast(testFolder, "test-resources")}]/`
			);
			if (resourcesPattern.test(url)) {
				return url.replace(resourcesPattern, `/base/${srcFolder}/`);
			} else if (testResourcesPattern.test(url)) {
				return url.replace(testResourcesPattern, `/base/${testFolder}/`);
			}
		}
