...
class Framework {
	...
	autoDetectType() {
		...
		if (fileContent) {
			...
			if (!configs || configs.length === 0) {
				karma.log("error", ["Could not parse ui5.yaml. Please make sure that the ui5.yaml has a valid format. For reference check: https://github.com/SAP/ui5-project/blob/master/docs/Configuration.md#configuration"]);
			}

			if (!configs[0].type) {
				karma.log("error", ["Project doesn't have a type configured. Please make sure that the ui5.yaml has a type declared. For reference check: https://github.com/SAP/ui5-project/blob/master/docs/Configuration.md#configuration"]);
			}
	...
	rewriteUrl(url) {
		...
		if (!type) {
			...
			karma.log("error", [`Failed to rewrite url. Type "${type}" is not supported. Please use "library" or "application" as type.`]);
		}
