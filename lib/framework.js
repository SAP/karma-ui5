...
class Framework {
	...
	async init({config, logger, filesPromise}) {
		...
		let projectTree;
		if (config.ui5.serveFiles !== false) {
			projectTree = await project.getProjectTree(config);
		}
		...
		if (projectTree) {
			...
			await ui5Middleware.initWithKarmaFiles({
				middleware: this.beforeMiddleware,
				tree: projectTree,
				basePath: config.basePath,
				type: config.ui5.type,
				paths: config.ui5.paths,
				filesPromise,
				// Also serve dependencies when no proxy url is configured
				serveDependencies: !config.ui5.url,
				log
			});
			if (config.ui5.url) {
				// If url is passed, use proxy to load dependencies (after karma or custom resources middleware)
				const proxyMiddleware = require("./middleware/proxy");
				await proxyMiddleware.init({
					basePath: "/base/",
					middleware: this.beforeMiddleware,
					targetUrl: config.ui5.url,
					log
				});
			}
		} else {
			...
			if (!config.ui5.url) {
				const ui5Middleware = require("./middleware/ui5");
				await ui5Middleware.initWithoutKarmaFiles(this.middleware, projectTree);
			} else {
				// If url is passed, use proxy to load dependencies (after karma or custom resources middleware)
				const proxyMiddleware = require("./middleware/proxy");
				await proxyMiddleware.init({
