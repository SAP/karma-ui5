...
class Framework {
	...
	rewriteUrlForKarma(url) {
		...
		if (!type) {
			...
			const srcResourcesPattern = new RegExp(`^/base/${this.replaceLast(srcFolder, "resources/")}`);
			const srcTestResourcesPattern = new RegExp(`^/base/${this.replaceLast(srcFolder, "test-resources/")}`);
			const testResourcesPattern = new RegExp(`^/base/${this.replaceLast(testFolder, "resources/")}`);
			const testTestResourcesPattern = new RegExp(`^/base/${this.replaceLast(testFolder, "test-resources/")}`);
	...
	pauseRequests() {
		return (req, res, next) => {
			if (req.url) { // TODO: Some unit tests don't supply req.url
				if (req.url.startsWith(`/base/${this.replaceLast(this.config.ui5.paths.src, "test-resources/")}`)) {
					console.log("src: " + this.config.ui5.paths.src);
					console.log("test: " + this.config.ui5.paths.test);
					console.log("IN : " + req.url);
					req.url = this.rewriteUrlForKarma(req.url);
