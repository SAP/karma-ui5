...
const rewriteUrl = {
	toFileSystem: (config) => {
		return (req, res, next) => {
			req.url = rewriteUrl._rewriteToFileSystem(req.url, config);
			next();
		};
	},
	toVirtual: (config) => {
		return (req, res, next) => {
			req.url = rewriteUrl._rewriteToVirtual(req.url, config);
			next();
		};
	},
	_rewriteToFileSystem: function(url, config) {
	...
	_rewriteToVirtual: function(url, config) {
		const {type, paths} = config.ui5 || {};
		if (!type) {
		// TODO: do we want no type to be allowed?
			return url;
	...
	_rewriteToVirtualWithoutBase: function(url, {type, paths}) {
		if (type === "application") {
			const webappFolder = paths.webapp;
			const webappPattern = new RegExp(`^/${webappFolder}/`);
			if (webappPattern.test(url)) {
				return url.replace(webappPattern, "/");
			}
		} else if (type === "library") {
			const srcFolder = paths.src;
			const testFolder = paths.test;
			const srcPattern = new RegExp(`^/${srcFolder}/`);
			const testPattern = new RegExp(`^/${testFolder}/`);
			if (srcPattern.test(url)) {
				return url.replace(srcPattern, "/resources/");
			} else if (testPattern.test(url)) {
				return url.replace(testPattern, "/test-resources/");
			}
			const srcResourcesPattern = new RegExp(`^/${replaceLast(srcFolder, "resources/")}`);
			const srcTestResourcesPattern = new RegExp(`^/${replaceLast(srcFolder, "test-resources/")}`);
			const testResourcesPattern = new RegExp(`^/${replaceLast(testFolder, "resources/")}`);
			const testTestResourcesPattern = new RegExp(`^/${replaceLast(testFolder, "test-resources/")}`);

			if (srcResourcesPattern.test(url)) {
				return url.replace(srcResourcesPattern, `/resources/`);
			} else if (srcTestResourcesPattern.test(url)) {
				return url.replace(srcTestResourcesPattern, `/test-resources/`);
			} else if (testResourcesPattern.test(url)) {
				return url.replace(testResourcesPattern, `/resources/`);
			} else if (testTestResourcesPattern.test(url)) {
				return url.replace(testTestResourcesPattern, `/test-resources/`);
			}
		}
		return url;
	}
